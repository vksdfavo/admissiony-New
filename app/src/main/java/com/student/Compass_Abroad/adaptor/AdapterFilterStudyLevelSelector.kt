...
class AdapterFilterStudyLevelSelector(
    private val requireActivity: FragmentActivity,
    private val arraylist: ArrayList<com.student.Compass_Abroad.modal.studyLevelModel.Data>,
    private val popupBinding: View
) : RecyclerView.Adapter<AdapterFilterStudyLevelSelector.MyViewHolder>() {

    private var filteredList: MutableList<com.student.Compass_Abroad.modal.studyLevelModel.Data> = arraylist.toMutableList() // Maintain filtered list
    private var selectedItems: MutableList<com.student.Compass_Abroad.modal.studyLevelModel.Data> = mutableListOf()

    var onItemClickListener: ((List<com.student.Compass_Abroad.modal.studyLevelModel.Data>) -> Unit)? = null
    ...
    fun getSelectedItems(): List<com.student.Compass_Abroad.modal.studyLevelModel.Data> {
        return selectedItems
    }

    fun setInitialSelection(selectedItems: List<com.student.Compass_Abroad.modal.studyLevelModel.Data>) {
    ...
    inner class MyViewHolder(private val binding: ItemCustomPopupCountrySelectorBinding) : RecyclerView.ViewHolder(binding.root) {
        ...
        fun bind(item:com.student.Compass_Abroad.modal.studyLevelModel.Data) {
    ...
    private val exampleFilter = object : Filter() {
        override fun performFiltering(constraint: CharSequence?): FilterResults {
            ...
            filteredList = if (filterPattern.isEmpty()) {
                ...
                arraylist.filter { it.label.toLowerCase().contains(filterPattern) }.toMutableList()
            }
        ...
        override fun publishResults(constraint: CharSequence?, results: FilterResults?) {
            filteredList = results?.values as? MutableList<com.student.Compass_Abroad.modal.studyLevelModel.Data> ?: mutableListOf()
            notifyDataSetChanged()

            // Handle "No results found" scenario
            popupBinding.findViewById<TextView>(R.id.tvSelect_noData).apply {
                visibility = if (filteredList.isEmpty()) View.VISIBLE else View.GONE
                text = "No Study Level Found"
            }
        }
    }
}